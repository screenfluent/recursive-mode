...
Each audited phase stores one canonical current ledger at `/.recursive/run/<run-id>/evidence/reviews/<phase-key>/<review-id>/ledger.md`, completed hash-bound snapshots at `/.recursive/run/<run-id>/evidence/reviews/<phase-key>/<review-id>/passes/<NNNN>.md`, and one hash-bound same-pass bundle at `/.recursive/run/<run-id>/evidence/review-bundles/<phase-key>/<review-id>/<NNNN>.md`. The ledger is current operational state, pass snapshots are protocol-level append-only history, and the bundle is reproducible review input rather than a second ledger. The chain is tamper-evident for partial or accidental drift while its hashes and references remain intact; it is not cryptographically append-only against an actor that can rewrite the entire run. An external anchor, protected Git history, or controller-owned storage would provide that stronger guarantee separately.
...
Reviewer and self-audit output has exactly `## Review Scope`, `## Findings`, and `## Verdict`. Every technical issue is a protocol-level append-only stable `F-*` record; previous IDs and fixed technical fields never disappear or change in a valid later ledger. Once a finding leaves `open`, its entire record is fixed in every valid later working ledger and pass, including all disposition-specific, claim, destination, ownership, approval, decision, and controller-verification fields. `Depends on` names only a true repair prerequisite, forms an acyclic graph over existing findings, and permits `fixed` only after every dependency is itself `fixed`.
...
`scheduled` is available only inside an active run and only for work already owned by a strictly later canonical phase. Scheduling uses a separate complete canonical phase-key registry, including `phase-5` for non-ledger Phase 5. It copies the full obligation into `/.recursive/run/<run-id>/evidence/reviews/scheduled/<owner-phase-key>/inventory.md`; `Source Ledger` and `Destination` must be canonical in-run paths with no lexical escape, symlink, reparse point, or junction, and invalid addresses are rejected before any read. A valid pending record makes that owner required. Consumption requires an existing canonical regular non-symlink owner artifact with the exact `<review-id>/F-*` backreference plus a canonical regular non-symlink evidence path and matching SHA-256. Invalid consumption continues to activate the true canonical owner derived from the verified source finding or scheduling directory, never from a corrupted copied `Owner phase`, and blocks validation so the obligation cannot disappear. Final verification rejects any pending or invalidly consumed record. Standalone review cannot schedule work.

Before repair changes a reviewed surface, advance to the next working pass from the immediately preceding completed snapshot, verify its normalized hash, preserve all terminal history, reset only open claims, and refresh the hash-bound bundle. Every changed diff byte, file membership or mode, artifact, or evidence reference requires that next pass; `/.recursive/scripts/recursive_review_surface.py` binds the current changed surface and local reviewed/evidence files without treating historical snapshots as current worktree claims. Ledger `Diff Basis` must be the exact same-pass hash-bound bundle; `Changed Files` must exactly equal the snapshot's changed-path projection; `Evidence Basis` must exactly equal the bundle followed by the snapshot's reference-path projection. Partial overlap is invalid. A working-tree bundle records both exact fixed JSON argv operations used to compute membership: tracked `diff --name-only -z <baseline> --` and untracked `ls-files --others --exclude-standard -z`; commit comparisons record the exact tracked range and no untracked operation. These are argv under a fixed Git executable boundary, never shell text. Explicit reviewed and evidence references and hash-bound bundle components must resolve directly without symlink, reparse point, or junction indirection; directories, missing paths, and other non-regular references are invalid, while changed-surface symlinks bind their link state and target text.

`recursive-review-bundle` auto-discovers relevant addenda by default. Do not silently omit them from delegated review context. The review scope cites the bundle plus its grounded upstream artifacts, addenda, changed files, code references, and evidence. Persisted repair/review action records cite the ledger, hash-bound bundle, and four-digit pass, and contain only sorted claims for existing open findings; `/.recursive/scripts/recursive_review_action.py` owns that claim grammar.
...
- Reviewer output has exactly `## Review Scope`, `## Findings`, and `## Verdict`; every technical issue is a protocol-level append-only stable `F-*` record
...
`00-worktree.md` is the source of truth for diff basis. `Baseline reference` records the human-facing source ref chosen in Phase 0. `Normalized baseline` records the exact commit that later audits execute against. `Comparison reference` records the intended comparison target. `Normalized diff command` is the human-readable tracked-comparison summary derived from those values; generated review bundles additionally record exact fixed JSON argv for tracked and untracked membership, and those argv fields are authoritative for the complete snapshot.
