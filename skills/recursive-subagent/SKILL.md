...
11. for every audited phase, the protocol path, immutable per-pass bundle, canonical ledger, and exact `recursive-review` output contract: `## Review Scope`, `## Findings`, `## Verdict`, with stable `F-*` records
...
Any change to the reviewed diff, artifact, or evidence basis requires a refreshed bundle and the next `recursive-review` pass before re-audit.

Repair agents return candidate claims only (`none|fixed|blocked`). Reports and action records remain claims; the controller verifies every row before writing a terminal disposition.
...
- [ ] Verify `Current Artifact` points at the reviewed artifact and refresh the bundle, ledger, and pass after any reviewed-surface change
...
- [ ] For every audited phase, verify the output contains exactly `## Review Scope`, `## Findings`, and `## Verdict`; all technical issues are stable `F-*` records in the cited ledger
- [ ] Verify the output includes requirement/plan alignment comments where relevant
- [ ] Verify the output ends with a clear verdict
- [ ] The controller verifies every finding row and candidate claim against the actual worktree diff, actual artifacts, owning contract, and named check; only the controller may assign a terminal disposition
...
Use for code review and high-risk Phase 3/4 audits.
...
```md
Review Protocol Path: `/.agents/skills/recursive-review/references/finding-protocol.md`
...
Phase: `<audited phase>`
Artifact path: `/.recursive/run/<run-id>/<audited-artifact>.md`
Role: `analyst`
Audit execution expectation: use exactly `## Review Scope`, `## Findings`, and `## Verdict`; emit every technical issue as a stable `F-*` record in the named ledger
...
- cite the canonical `Review Ledger` and return stable finding claims as `F-ID=none|fixed|blocked` for `recursive-subagent-action`
- prefer the stable reviewed artifact for `Current Artifact` when recording review/audit work
- cite upstream artifacts reread, relevant addenda, prior recursive evidence, changed files or code refs reviewed, claimed changes, and claimed verification per finding
```
...
Then open or advance the same canonical ledger, preserve prior IDs, and complete the full audit loop locally:
...
4. record every technical issue as an append-only `F-*` row
5. repair the work
6. rerun the audit
7. let the controller terminalize every row and set whole-ledger PASS only when the phase is actually ready to lock
...
- for an audited phase, its `## Review Scope` does not cite changed files or code refs
